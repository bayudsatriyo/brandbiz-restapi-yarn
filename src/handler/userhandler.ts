...
class Userhandler {
  ...
  async getUserByUsernameHandler(
    req: Request,
    res: Response,
    next: NextFunction
  ): Promise<void> {
  ...
  async updateUserHandler(
    ...
    try {
      ...
      if (req.file !== undefined) {
        image = req.format;
        dataUser.profileUrl = `${process.env.BASE_URL}/brandbiz/user/image/${image}`;
      } else {
  ...
  async userUpdateSkor(req: Request, res: Response, next: NextFunction) {
    try {
      ...
      const result = await userService.updateSkorLearningPath(
        useremail,
        idLearning,
        skorUser
      );
